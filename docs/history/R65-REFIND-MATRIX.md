# leap16-r65 rEFInd matrix + audit ledger
...
- **TEST-COVERED / HW-PENDING** — implemented in r64/r65 and covered by shell/synthetic regressions, but not yet proven by a real rEFInd boot transaction.
...
Existing three-loader evidence therefore remains **6/6 HW-PROVEN**. r64/r65 add six new directed rEFInd edges, making the implemented four-loader graph **12/12**, with the six new edges hardware-pending.
...
Existing three-loader restore evidence remains **6/6 HW-PROVEN**. r64/r65 implement all six rEFInd restore edges, producing a code-complete **12/12** cross-loader restore graph, pending hardware proof for the new six.
...
r65 rEFInd edges fail closed unless all of these are true:
...
r65 first uses an already-installed `refind` RPM/tool set. Otherwise it attempts:

`zypper --non-interactive install --no-recommends refind`
...
The local path must be an absolute, regular non-symlink file and pass `rpm -K`; installation still goes through zypper. r65 never downloads an unpinned rEFInd RPM itself.
